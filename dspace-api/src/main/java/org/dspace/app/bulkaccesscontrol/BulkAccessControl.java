...
public class BulkAccessControl extends DSpaceRunnable<BulkAccessControlScriptConfiguration<BulkAccessControl>> {
    ...
    public void internalRun() throws Exception {
        ...
        try {
            ...
            dso.getResourcePolicies().forEach(d->{
                ...
                this.authorizeService.addPolicies();
                this.authorizeService.createResourcePolicy(context, obj, d.getGroup(), null, Constants.READ,
    ...
    public  void  updatebitstreaorItemPolicy(List<ResourcePolicy> resourcePolicies ,BulkAccessControlInput accessControl) throws SearchServiceException, SQLException, AuthorizeException {
        //if(accessControl.getItem() != null)
        updateItemsAndBitstreamsPolicesFromDso(resourcePolicies,accessControl);
    ...
    private void updateItemsAndBitstreamsPolicesFromDso(List<ResourcePolicy> resourcePolicys,BulkAccessControlInput accessControl)
        ...
        itemIterator.stream().forEach(item->{
            ...
            if (Objects.nonNull(accessConditionItem)) {
                resourcePolicys.forEach(d->{
                    if (Objects.nonNull(accessControl.getItem()) && d.getAction() ==Constants.DEFAULT_ITEM_READ) {
                        updateItemPolicies(d,item,accessControl.getItem().getMode());//
                    }
                    if (Objects.nonNull(accessControl.getBitstream()) && d.getAction() ==Constants.DEFAULT_BITSTREAM_READ) {
                        updateBitstreamsPoliciesDso(item, d,accessControl); //
                    }
                });
    ...
    private Iterator<Item> findItems(String query, int start, int limit)
        ...
        DiscoverQuery discoverQuerytotalResult = buildDiscoveryQuery(query, start, 0);
        var total_searchresult= searchService.search(context, discoverQuerytotalResult).getTotalSearchResults();
        System.out.println("total_searchresult:::"+total_searchresult);
        DiscoverQuery discoverQuery = buildDiscoveryQuery(query, start, (int) total_searchresult);
    ...
    private List<Item> findItemsList(String query, int start, int limit)
            throws SearchServiceException {
        DiscoverQuery discoverQuerytotalResult = buildDiscoveryQuery(query, start, 0);
        var total_searchresult= searchService.search(context, discoverQuerytotalResult).getTotalSearchResults();
        System.out.println("total_searchresult:::"+total_searchresult);
    ...
    private void updateBitstreamsPoliciesDso(Item item, ResourcePolicy resourcePolicy,BulkAccessControlInput accessControl) {
        //AccessConditionBitstream.Constraint constraintss= accessControl.getBitstream().getConstraints();
        ...
        item.getBundles(CONTENT_BUNDLE_NAME).stream()
                .flatMap(bundle -> bundle.getBitstreams().stream())
                .forEach(bitstream -> {
                    updateItemPolicies(resourcePolicy,bitstream,accessControl.getBitstream().getMode());
                    logInfoDso(accessControl.getBitstream().getMode(),bitstream);
                    //set by vipul to remove defaul one
                     /* try {
    ...
    private void removeAllPolicies(DSpaceObject dso) {
        try {
            resourcePolicyService.removeAllPolicies(context, dso);
        } catch (SQLException | AuthorizeException e) {
            throw new BulkAccessControlException(e);
        }
    }
    ...
    private void logInfoDso( String mode, DSpaceObject dso) {
        ...
        message.append(mode.equals(ADD_MODE) ? "Adding " : "Replacing ")
                ...
                .append("access conditions:::::::" +dso.getType());
